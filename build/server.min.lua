...
a["server.main"]=function(...)local n=i("shared.gcm")if not n.running then
error("Main entrypoint encountered without GCM running!")end;local s=i("shared.socket")
local h=i("shared.logger")local r=i("shared.apierrors")local d=i("shared.validate")
local l=d.types;local u=i("server.game")local c={}
s.listen({service="sccriblio"},function(m)local f
m:handleDisconnect(function()
if f then
f:removePlayer(m)if f:isEmpty()then c[f.code]=nil end end end)
local function w()local y=4;local p=0
while true do local v=""
for b=1,y do v=v..
string.char(math.random(string.byte("A"),string.byte("Z")))end;if not c[v]then return v end;p=p+1;if p%5 ==0 then y=y+1 end end end
m:handle("newgame",function(y,p)if f then return y.fail(r.AINGAME)end
local v,b=d(p,{title={l.String(32,1)},name={l.String(14,1)},color={l.Color(),l.Min(2)},private={l.Bool()}})if not v then return y.fail(b)end;local g=w()
f=u.new(p.title,p.private,g)f:addPlayer(p.name,p.color,m)c[g]=f
m:emit("message",{author={},content={color=colors.lightGray,text=
"Use the code: '"..g.."' to invite others"}})end)
m:handle("joingame",function(y,p)if f then return y.fail(r.AINGAME)end
local v,b=d(p,{code={l.String(nil,1)},name={l.String(14,1)},color={l.Color(),l.Min(2)}})if not v then return y.fail(b)end;if c[p.code]then f=c[p.code]
f:addPlayer(p.name,p.color,m)else return y.fail(r.NEXISTS)end end)
m:handle("guess",function(y,p)if not f then y.fail(r.NINGAME)end
local v,b=d(p,{text={l.String(64,1)}})if not v then return y.fail(b)end;if f.word then end
f:broadcast(m,p.text)return y.succeed()end)
m:handle("games",function(y)local p={}
for v,b in pairs(c)do if not b.private then
table.insert(p,{code=v,title=b.title,players=#b.players})end end;y.succeed(p)end)end)end
a["server.game"]=function(...)local n={}local s=i("shared.logger")function n.new(h,r,d)
local l={title=h,private=r,code=d,players={}}
setmetatable(l,{__index=n,__tostring=n.tostring})return l end
function n:addPlayer(h,r,d)
table.insert(self.players,{name=h,color=r,sock=d})self:broadcast(nil,"+"..h.." joined")end
function n:removePlayer(h)for r=1,#self.players do
if
self.players[r].sock.uuid==h.uuid then table.remove(self.players,r)return end end
s.warn("removePlayer was called with a player that wasn't in the game: ",h.uuid)end;function n:isEmpty()return#self.players==0 end
function n:broadcast(h,r)local d;if h then
for u=1,#
self.players do if self.players[u].sock.uuid==h.uuid then
d=self.players[u]break end end end;local l=colors.gray;if
not d then d={}l=colors.lightGray end;for u=1,#self.players do
self.players[u].sock:emit("message",{author={name=d.name,color=d.color},content={color=l,text=r}})end end;return n end
a["server.bootstrap"]=function(...)local n=i("shared.logger")
n.init({i("shared.logger.backends.console").new()},{level=n.LogLevels.INFO})local s=i("shared.gcm")
n.info("Bootstrapping application...")s:run("server.main")n.destroy()end;return a["server.bootstrap"](...)